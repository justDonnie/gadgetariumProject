...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query("select new peaksoft.dto.ProductResponse(p.id,p.name,p.price,p.characteristic,p.madeIn,p.brand,p.category)from Product p")
    List<ProductResponse>getAllProducts();

    Optional<ProductResponse>findProductById(Long prodId);

    @Query("select new peaksoft.dto.ProductResponse(p.id,p.name,p.price, p.characteristic,p.madeIn,p.brand,p.category) from Product p where p.id= :productId")
